...
class Sharding:
    ...
    def load_sentences(self):
        ...
        #Joining Sentences to create "articles" (sentence groupings)
        ...
        for i in tqdm(range((len(sentences) // group_size) + 1)):
            article_sentences = sentences[i*group_size:(i+1)*group_size]
            self.articles[i] = '\n'.join(article_sentences)
            self.sentences[i] = article_sentences
            global_sentence_count += len(article_sentences)

        assert global_sentence_count == len(sentences)

        print("End: Sentence Loading")
