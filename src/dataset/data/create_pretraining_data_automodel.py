...
def main():
    ...
    print("Custom Tokenizer {}".format(args.custom_tokenizer))
    tokenizer = AutoTokenizer.from_pretrained(args.bert_model, max_len=args.max_seq_length)
    ...
    document_file = '/'.join(output_file.split('/')[:-1]) + '/' + input_files[0].split('/')[-1] + '.p'
    if os.path.exists(document_file):
        documents = pickle.load(open(document_file,'rb'))
    else:
        documents = create_training_documents(
            input_files,
            tokenizer,
            rng
        )
        pickle.dump(documents, open(document_file, 'wb'))
