...
def pretrain_validation(args, model, validation_dataset, step):
    ...
    for _, batch in enumerate(tqdm(data_batches, smoothing=1)):
        ...
        labels[torch.where(labels == -1)] = -100

        total_loss = model.forward(input_ids,
                                   attention_mask,
                                   token_type_ids,
                                   None,
                                   None,
...
def train(
    ...
    torch.cuda.synchronize()
    dist.barrier(model.data_parallel_group)
...
def prepare_model_and_optimizer(args):
    ...
    optimizer_grouped_parameters = prepare_optimizer_parameters(
        args, model
    )
...
def start_training(args, model, optimizer, lr_scheduler, start_epoch):
    ...
    for index in range(start_epoch, args.num_epochs):
        ...
        if global_step >= int(args.max_steps * args.start_long_phase_perc):
            pretrain_dataset_provider.switch_to_long_phase(int(args.train_micro_batch_size_per_gpu/4))
        ...
        if (
            ...
            save_training_checkpoint(
                ...
                ckpt_id="checkpoint_epoch_{}".format(index+1),
            )
            dist.barrier()
    logger.info(
...
def main():
    ...
    checkpoints = glob(args.load_training_checkpoint + '/*checkpoint*')

    model, _, _ = prepare_model_and_optimizer(args)
    model_hf, tokenizer = prepare_hf_model_and_tokenizer(args)

    for checkpoint in checkpoints:
        checkpoint_id = checkpoint.split('/')[-1]
        ...
        model_hf.load_state_dict(model.module.state_dict())
        model_hf.save_pretrained(args.output_dir+'/'+checkpoint_id)
        tokenizer.save_pretrained(args.output_dir+'/'+checkpoint_id)
