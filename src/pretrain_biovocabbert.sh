deepspeed --master_port 7002 run_pretraining_automodel.py \
  ...
  --tokenizer_name ../output/biovocabbert_tokenizer \
  --lr 6e-4 \
  ...
  --max_steps 63000 \
  --dataset_path /dataset/data/biovocabbert \
  --output_dir ../output/biovocabbert_temp \
  --print_steps 1000 \
...
deepspeed --master_port 7003 --include="localhost:5" model_load.py \
  ...
  --max_steps 63000 \
  --dataset_path /dataset/data/biovocabbert \
  --output_dir ../output/biovocabbert \
  --print_steps 1000 \
  --num_epochs_between_checkpoints 50 \
  --job_name pretraining_experiment \
  --project_name budget-bert-pretraining \
  ...
  --layer_norm_type pytorch \
  --start_long_phase_perc 0.9 \
  --load_training_checkpoint ../output/biovocabbert_temp/pretraining_experiment-
  
rm -r ../output/biovocabbert_temp