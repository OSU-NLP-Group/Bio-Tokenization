...
class ValidationDataset:
    def __init__(self, args):
        ...
        self.dataset_files = [
            ...
            if os.path.isfile(os.path.join(dataset_path, f)) and "test" in f and f.endswith('hdf5')
        ]
        ...
        self.short_seq_files = []
        self.long_seq_files = []
        self.long_phase_percentage = 0.9

        for file in self.dataset_files:
            file_index = int(file.split('test_shard_')[1].split('.')[0])
            if file_index < int(self.long_phase_percentage*self.num_files):
                self.short_seq_files.append(file)
            else:
                self.long_seq_files.append(file)

        self.dataset_files = self.short_seq_files
        self.num_files = len(self.dataset_files)
...
class PreTrainingDataset(BertDatasetProviderInterface):
    def __init__(self, args, data_prefix="train", logger=None):
        ...
        self.dataset_files = [
            os.path.join(dataset_path, f)
            for f in os.listdir(dataset_path)
            if os.path.isfile(os.path.join(dataset_path, f)) and data_prefix in f and f.endswith('hdf5')
        ]
        ...
        # Choosing Shorter or Longer Instances (After 90% of training use longer examples)
        self.long_phase_percentage = 0.9
        ...
        for file in self.dataset_files:
            file_index = int(file.split('train_shard_')[1].split('.')[0])
            if file_index < int(self.long_phase_percentage*self.num_files):
                self.short_seq_files.append(file)
            else:
                self.long_seq_files.append(file)
    ...
    def switch_to_long_phase(self, new_batch_size):
        self.dataset_files = self.long_seq_files
        self.num_files = len(self.dataset_files)

        self.train_micro_batch_size_per_gpu = new_batch_size
